...
class NaiveBayes:
    ...
    def update_model(self, bow, label):
        ...
        for word in bow:
            ...
            if word in self.class_word_counts[label]:
                self.class_word_counts[label][word] += bow[
                    word]  # if the word already exist in label add the number of the word
            else:
                self.class_word_counts[label][word] = bow[
                    word]  # if the word have not exist in label update the number of word
            self.class_total_word_counts[label] += bow[word]  # update the number of words seen for each label
        self.class_total_doc_counts[label] += 1  # update the number of documents seen of each label
        # no return void
    ...
    def tokenize_and_update_model(self, doc, label):
        ...
        """
        bow = self.tokenize_doc(doc.lower())  # tokenize document to lower case
        self.update_model(bow, label)  # update the model with the document and row
    ...
    def top_n(self, label, n):
        ...
        """
        sortedByCount = sorted(self.class_word_counts[label].items(),
    ...
    def p_word_given_label(self, word, label):
        ...
        """
        return  self.class_word_counts[label][word] / self.class_total_word_counts[label]  # p(w) = cw/total word
    ...
    def p_word_given_label_and_alpha(self, word, label, alpha):
        ...
        """
        return (self.class_word_counts[label][word] + alpha) / (self.class_total_word_counts[label] + alpha *len(self.vocab)) # 3.25
    ...
    def log_likelihood(self, bow, label, alpha):
        ...
        toReturn = 0.0
        for word in bow:
            toReturn += math.log(self.p_word_given_label_and_alpha(word, label, alpha)) * bow[word] # 3.13
        return toReturn
    ...
    def log_prior(self, label):
        ...
        """
        numOfDocInLabel = self.class_total_doc_counts[label]
        totalNumOfDoc = 0
        for lab in self.class_total_doc_counts:
            totalNumOfDoc += self.class_total_doc_counts[lab]  # 4.11
        return math.log(numOfDocInLabel / totalNumOfDoc) # 4.10
    ...
    def unnormalized_log_posterior(self, bow, label, alpha):
        ...
        bow - a bag of words (i.e., a tokenized document)
        """
        return self.log_prior(label) + self.log_likelihood(bow, label, alpha)  # 4.10
